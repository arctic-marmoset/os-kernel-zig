...
pub fn panic(message: []const u8, error_return_trace: ?*StackTrace, return_address: ?usize) noreturn {
    ...
    // TODO: Maybe use new DWARF unwind functions in stdlib.
    ...
    if (debug_info) |*info| {
        ...
        const debug_frame = info.section(.debug_frame).?;
...
fn getReturnAddress(
    ...
    switch (fp_rule) {
        ...
        .offset => |amount| {
            const fp_address = if (amount < 0)
                ...
                frame_address + @as(u64, @intCast(fp_rule.offset));
            const fp = mem.readInt(u64, @as(*const [8]u8, @ptrFromInt(fp_address)), .little);
    ...
    const ra_address = if (ra_rule.offset < 0)
        ...
        frame_address + @as(u64, @intCast(ra_rule.offset));
    const ra = mem.readInt(u64, @as(*const [8]u8, @ptrFromInt(ra_address)), .little);
