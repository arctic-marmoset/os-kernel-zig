...
pub fn executeInstructions(instructions: []const Instruction, table: *std.ArrayList(CfiRow)) !void {
    ...
    while (offset < instructions.len) {
        ...
        const executed_count = try executeInstructionsForRow(instructions[offset..], table.items[0], &row);
...
pub fn executeAllInstructionsForRow(instructions: []const Instruction, first_row: CfiRow, row: *CfiRow) !void {
    const executed_count = try executeInstructionsForRow(instructions, first_row, row);
...
fn executeInstructionsForRow(
    ...
    for (instructions) |instruction| {
        switch (instruction) {
            ...
            .restore => |register| {
                row.registers[register] = first_row.registers[register];
            },
            .def_cfa => |cfa| {
...
pub const CfiRow = struct {
    ...
    pub const fp_index = 6;
    pub const sp_index = 7;
    pub const ra_index = 16;

    pub const column_count = register_count + 1;
};
...
pub fn printFde(stream: anytype, fde: Fde) !void {
    ...
    const column_titles = "LOC              CFA      " ++ comptime generateRegisterTitles(CfiRow.column_count) ++ "\n";
